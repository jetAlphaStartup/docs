...
## Get Price Difference Between Sell and Buy Prices

Get the profit margin (difference) between sell and buy prices for a cluster. Returns profit in either currency or percentage format, grouped by sign combinations (pp, nn, np). This endpoint is useful for analyzing profit margins and pricing strategies.
...
**Mode Options:**
- `currency` - Returns profit as the difference in currency units (sellPrice - buyPrice)
- `percentage` - Returns profit as a percentage ((sellPrice - buyPrice) / buyPrice * 100)
...
<AccordionGroup>
  <Accordion title="Currency Mode" icon="dollar-sign">
    ...
    **Special Cases:**
    - If only sell price exists: `profit = sellPrice`
    - If only buy price exists: `profit = buyPrice`
    - If neither exists: `profit = 0`
    - If both exist: `profit = sellPrice - buyPrice`
    
    **Example:** If sell price is 800 and buy price is 600, profit = 200
  </Accordion>
  ...
  <Accordion title="Percentage Mode" icon="percent">
    ...
    **Special Cases:**
    - If only sell price exists: `profitPercentage = 100%` (100% profit margin)
    - If only buy price exists: `profitPercentage = 100%` (100% profit margin)
    - If neither exists: `profitPercentage = 0%`
    - If both exist and buyPrice ≠ 0: `profitPercentage = ((sellPrice - buyPrice) / buyPrice) * 100`
    - If buyPrice = 0: Division by zero is avoided, returns appropriate default
    
    **Example:** If sell price is 800 and buy price is 600, profit = ((800 - 600) / 600) * 100 = 33.33%
  </Accordion>
  ...
  <Accordion title="Missing Prices" icon="alert-circle">
    **Handling Missing Prices**
    ...
    - **Only sell price exists**: Treated as full profit (currency: sellPrice, percentage: 100%)
    - **Only buy price exists**: Treated as potential cost (currency: buyPrice, percentage: 100%)
  ...
  <Accordion title="Sign Combinations" icon="layers">
    **Sign Combination Grouping**
    ...
    Only non-empty combinations are included in the response. Axes are merged from both sell and buy price tables to ensure complete coverage.
  </Accordion>
</AccordionGroup>

<Note>
  The response format uses array structure (x, y, value) optimized for frontend table rendering. The `x` coordinate represents SPH value, `y` coordinate represents CYL value, and `value` represents the calculated profit. Axes are automatically merged from both sell and buy price tables to include all combinations where either price exists.
</Note>
...
<CodeGroup>
```bash cURL
curl -X GET "https://joptic.jethings.com/lens-pricing/diff?cluster=1.56%20HMC&mode=currency" \
  -H "x-store-id: your-store-id" \
  -H "Authorization: Bearer <token>"
```
...
<CodeGroup>
```bash cURL
curl -X GET "https://joptic.jethings.com/lens-pricing/diff?cluster=1.56%20HMC&mode=percentage" \
  -H "x-store-id: your-store-id" \
  -H "Authorization: Bearer <token>"
```
</CodeGroup>
...
- `400 Bad Request` - Missing or invalid cluster parameter, invalid mode value
- `401 Unauthorized` - Missing or invalid authentication token
- `403 Forbidden` - User does not have access to the store
- `404 Not Found` - Price list not found for sell or buy type
- `500 Internal Server Error` - Server error

<Info>
  This endpoint requires both sell and buy price lists to be configured for the store. The system automatically fetches prices from both lists and calculates the difference. If either price list is missing, appropriate error responses are returned.
</Info>
