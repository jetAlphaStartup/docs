...
GET `/pos/products/:pricelistId`

Retrieve a paginated list of products from a store's inventory filtered by price list. Supports search functionality to filter products.
...
|-----------|------|----------|-------------|
| `pricelistId` | string | Yes | The price list ID to filter products by |
...
- `404 Not Found`: Store not found or price list not found

### Example Requests

**Get all products:**
```bash
curl -X GET "https://jethings-backend.fly.dev/pos/products/YOUR_PRICELIST_ID" \
...
```bash
curl -X GET "https://jethings-backend.fly.dev/pos/products/YOUR_PRICELIST_ID?search=B001" \
  -H "x-store-id: YOUR_STORE_ID" \
  -H "Authorization: Bearer YOUR_ACCESS_TOKEN"
```
...
```bash
curl -X GET "https://jethings-backend.fly.dev/pos/products/YOUR_PRICELIST_ID?search=Nike" \
  -H "x-store-id: YOUR_STORE_ID" \
  -H "Authorization: Bearer YOUR_ACCESS_TOKEN"
```
...
- Products are filtered by the specified price list ID in the path parameter
- The price list ID must be valid and associated with the store
- Product prices shown will reflect the pricing from the specified price list
- If the price list doesn't exist or isn't associated with the store, a 404 error will be returned

### Search Functionality
- The search parameter filters products across multiple fields (e.g., barcode, name, reference)
- Search is case-insensitive and supports partial matching
- If no search parameter is provided, all products in the store (filtered by price list) are returned (with pagination)
